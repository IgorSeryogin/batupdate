		...
		then
			if [ "$INTERNET_CONNECTED" = "y" ];
				then
					...
					local VALIDATION=`echo "$ACTUAL_VERSION" | grep -E "^[0-9]{4}\.[0-9]{1,2}\.[0-9]$"`
					if [ -z "$VALIDATION" ];
						then ACTUAL_VERSION="ERROR";
					fi
				else
					ACTUAL_VERSION="NA";
				fi
...
CheckDispatcherVersion () {
	...
	if [ -e "$DISPATCHER" ];
		...
		if [ -z "$DISPATCHER_VERSION" ]; 
			then DISPATCHER_VERSION="ERROR";
		fi
		else
			DISPATCHER_VERSION="NA";
	fi	
...
PrintStat () {
	echo "===================================="
	if [ "$INTERNET_CONNECTED" = "y" ];
		then EchoGreen "Internet Ready!";
		else EchoRed "Internet Not Ready!";
	fi
	if [ "$READY_TO_BUILD" = "y" ];
		then EchoGreen "Build Ready!";
		else EchoRed "Build Not Ready!";
	fi
	if [ "$ACTUAL_VERSION" != "NA" ] && [ "$ACTUAL_VERSION" != "ERROR" ];
		then
			EchoGreen "Actual Version: $ACTUAL_VERSION";
		else
			EchoRed "Actual Version: $ACTUAL_VERSION";
	fi
	if [ "$BATCTL_VERSION" = "NA" ] || ( [ "$ACTUAL_VERSION" != "NA" ] && [ "$ACTUAL_VERSION" != "ERROR" ] && [ "$ACTUAL_VERSION" != "$BATCTL_VERSION" ] );
		then
			EchoRed "Batctl Version: $BATCTL_VERSION";
		else
			EchoGreen "Batctl Version: $BATCTL_VERSION";
	fi
	if [ "$BATMAN_ADV_VERSION" = "NA" ] || ( [ "$ACTUAL_VERSION" != "NA" ] && [ "$ACTUAL_VERSION" != "ERROR" ] && [ "$ACTUAL_VERSION" != "$BATMAN_ADV_VERSION" ] );
		then
			EchoRed "Batman-adv Version: $BATMAN_ADV_VERSION";
		else
			EchoGreen "Batman-adv Version: $BATMAN_ADV_VERSION";
	fi
	if [ "$ALFRED_VERSION" = "NA" ] || ( [ "$ACTUAL_VERSION" != "NA" ] && [ "$ACTUAL_VERSION" != "ERROR" ] && [ "$ACTUAL_VERSION" != "$ALFRED_VERSION" ] );
		then
	...
	echo "===================================="
	if [ "$DISPATCHER_VERSION" != "NA" ] && [ "$DISPATCHER_VERSION" != "ERROR" ];
		then
...
InstallAlfred () {
	...
	if [ "$ALFRED_VERSION" = "$ACTUAL_VERSION" ]; 
		then
			EchoGreen "Alfred OK!";
		else
...
InstallLogo () {
	if [ -e /etc/mesh/batlogo_black.svg ];
		then
			EchoGreen "Logo OK!";
		else
...
InstallNetworkManagerDispatcher () {
	if [ -e /etc/NetworkManager/dispatcher.d/02batman-adv ];
		then
...
InstallConnection () {
	if [ -e "/etc/NetworkManager/system-connections/open-mesh.org" ];
		then
